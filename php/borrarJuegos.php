...
require 'jsonEsperandoBorrarVideojuego.php';
/*
...
if(isset($parameters)){
	...
	if(JSONCorrectoBorrarJuego($mensajeRecibido)){
		$juego = $mensajeRecibido["juegoBorrar"];
		$id = $juego["Id"];
		$query  = "DELETE FROM ficheritos WHERE id = $id";
		$result = $conn->query ( $query );
		if (isset ( $result ) && $result) { // Si pasa por este if, la query está está bien y se ha insertado correctamente
			$arrMensaje["estado"] = "ok";
			$arrMensaje["mensaje"] = "juego borrado";
